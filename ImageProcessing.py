...
class Image2Print:
    ...
    def __calc_factors(self):
        ...
        """
        self.printbedx_px = self.dimx * \
            self.dpix                           # Printbed pixel in x direction
        self.printbedx_px = round(self.printbedx_px.to_reduced_units())
        # Printbed pixel in y direction
        self.printbedy_px = self.dimy * self.dpiy
        self.printbedy_px = round(self.printbedy_px.to_reduced_units())
        # Offset in pixel in x direction
        self.offsetx_px = self.offsetx * self.dpix
        self.offsetx_px = round(self.offsetx_px.to_reduced_units())
        # Offset in pixel in y direction
        self.offsety_px = self.offsety * self.dpiy
    ...
    def __load_svg(self):
        ...
        if self.path_in != '':
            ...
            self.img = pyvips.Image.svgload(
                self.path_in, dpi=self.dpimax.magnitude)
            ...
            print('\nPixel width:', self.img.width, '\nPixel height:', self.img.height, '\nDPIx:', round(
                self.img.xres/(1/25.40), 3), '\nDPIy:', round(self.img.yres/(1/25.40), 3), '\n'),
            self.buffer = self.img.tiffsave_buffer(
                xres=self.dpmmy.magnitude, yres=self.dpmmy.magnitude)
    ...
    def __load_bitmap(self):
        ...
        if self.path_in != '':
            ...
            self.buffer = self.img.tiffsave_buffer(
                xres=self.dpmmx.magnitude, yres=self.dpmmy.magnitude)
            self.img = pyvips.Image.tiffload_buffer(self.buffer)
            return True
        else:
            print('\nNo image loaded')
    ...
    def __calc_image(self):
        ...
        self.scalex = round(
            (self.img.xres/self.dpmmmax.magnitude), 7)*self.sizex
        self.scaley = round(
            (self.img.yres/self.dpmmmax.magnitude), 7)*self.sizey
        print('Rescale image for DPIx and DPIy x:',
              self.scalex, 'y:', self.scaley)
        ...
        self.printbed = pyvips.Image.black(
            self.printbedx_px, self.printbedy_px)
        self.printbed = self.printbed.ifthenelse([0, 0, 0], self.bg)
        if self.bg == [255, 255, 255]:
            self.printbed = self.printbed.colourspace('b-w')

        print('Merge images with pixel offset X=',
              self.offsetx_px, 'and Y=', self.offsety_px)
        self.offsety_px = self.printbedy_px - self.img.height - self.offsety_px.magnitude
        self.printbed = self.printbed.insert(
            self.img, self.offsetx_px.magnitude, self.offsety_px.magnitude)
        ...
        self.printbed.tiffsave(self.path_out, squash=True, xres=self.dpmmx.magnitude,
                               yres=self.dpmmy.magnitude, compression='deflate')
        print('\n####     Done     ####\n'),
    ...
    def calculate_printimage(self,
        ...
        """Calculates a 1bit printimage bg = [255,255,255] or a printimage with a give printbed (bg color)
            ...
            Args:
                ...
                bg (list, optional): [Background colour -> Fraunhofer RGB [23,156,125]']. Defaults to [255,255,255].
        """
        self.__set_printsettings(
            dpix, dpiy, dimx, dimy, sizex, sizey, rot, offsetx, offsety, shrink, bg)
